...
def test_stop_transmitters(monkeypatch, transmitter_manager, mock_logger):
    ...
    transmitter_manager.interfaces = {
        "transmitter1": {"interfaceTrans": mock_interface, "stop_event": Mock()},
        "transmitter2": {"interfaceTrans": mock_interface, "stop_event": Mock()},
    }
    transmitter_manager.processes = {
        "transmitter1": mock_process,
        "transmitter2": mock_process,
    }
    ...
    # Check that the stop_event was set and interface stop was called
    for transmitter_id in transmitter_manager.interfaces:
        transmitter_manager.interfaces[transmitter_id]["stop_event"].set.assert_called_once()
        transmitter_manager.interfaces[transmitter_id]["interfaceTrans"].stop.assert_called_once()
