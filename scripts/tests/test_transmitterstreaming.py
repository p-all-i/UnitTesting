...
def mock_transmitter_config():
    return {
        'camera_type': 'Continuous Streaming',
...
def transmitter_interface(mock_redis, mock_shared_queue, mock_transmitter_config, mock_logger, mock_stop_event):
    instance = ContinuousStreamingTransmitter(
...
def test_configure_camera(mock_mv_camera, transmitter_interface):
...
@patch('assembly.transmitters.Transmitter_software.MvCamera')
def test_configure_camera_fail(mock_mv_camera, transmitter_interface):
    mock_camera_instance = mock_mv_camera.return_value
...
@patch('assembly.transmitters.Transmitter_software.MvCamera')
def test_stop(mock_mv_camera, transmitter_interface):
    # Setup mock camera
    mock_camera_instance = mock_mv_camera.return_value
    transmitter_interface.cam = mock_camera_instance
...
@patch('assembly.transmitters.Transmitter_software.redis.StrictRedis', autospec=True)
def test_redis_initialization(mock_redis, transmitter_interface):
    mock_redis_instance = mock_redis.return_value
    transmitter_interface.init_redis()
    assert transmitter_interface.redis_client is not None
    mock_redis_instance.pubsub.assert_called_once()


@patch('assembly.transmitters.Transmitter_software.redis.StrictRedis', autospec=True)
def test_redis_subscribe(mock_redis, transmitter_interface):
    mock_redis_instance = mock_redis.return_value
